      ...
      <section style={{ maxWidth: 1180, margin: "0 auto", padding: "56px 20px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1.1fr) minmax(320px, 0.9fr)", gap: 28 }}>
          <div>
            ...
            <h1 style={{ fontSize: 54, lineHeight: 1.02, margin: "0 0 18px", letterSpacing: "-0.04em" }}>
              Aplomo Systems
            </h1>

            <p style={{ fontSize: 19, lineHeight: 1.6, color: "#cbd5e1", maxWidth: 720 }}>
              Plataforma para patios industriales, stockpiles, GPS, dispositivos, datos, gobernanza, analítica e IA futura.
            </p>
          ...
          <aside style={{ border: "1px solid rgba(148,163,184,.35)", borderRadius: 22, padding: 20, background: "#ffffff", color: "#0f172a" }}>
            <div style={{ fontSize: 12, fontWeight: 900, color: "#475569", textTransform: "uppercase", letterSpacing: .8 }}>
              Acceso demo
            </div>
            ...
            {sessionEmail ? (
              <div style={{ border: "1px solid #bbf7d0", background: "#f0fdf4", color: "#166534", borderRadius: 14, padding: 12, marginBottom: 14 }}>
                Sesión actual: <strong>{sessionEmail}</strong>
              </div>
            ) : null}
            ...
            <select
              ...
              disabled={isLoading}
              style={{ width: "100%", border: "1px solid #cbd5e1", borderRadius: 12, padding: "11px 12px", marginBottom: 12 }}
            >
              {APLOMO_DEMO_ROLE_ACCOUNTS.map((account) => (
                <option key={account.accountKey} value={account.email}>
                  {account.displayName} · {account.accountScope === "platform" ? account.platformRole : account.tenantRole}
                </option>
              ))}
            </select>
            ...
            <input
              ...
              disabled={isLoading}
              placeholder="AplomoDemo-2026!"
              style={{ width: "100%", boxSizing: "border-box", border: "1px solid #cbd5e1", borderRadius: 12, padding: "11px 12px", marginBottom: 12 }}
            />

            {selectedAccount ? (
              <div style={{ border: "1px solid #e2e8f0", borderRadius: 14, padding: 12, background: "#f8fafc", marginBottom: 14 }}>
                <strong>{selectedAccount.displayName}</strong>
                <div style={{ fontSize: 13, color: "#475569", marginTop: 8 }}>{selectedAccount.description}</div>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <button type="button" onClick={() => void login()} disabled={isLoading} style={{ border: 0, borderRadius: 12, padding: "11px 14px", background: "#111827", color: "#ffffff", fontWeight: 900 }}>
                Iniciar sesión
              </button>

              {sessionEmail ? (
                <button type="button" onClick={() => void logout()} disabled={isLoading} style={{ border: "1px solid #fecaca", borderRadius: 12, padding: "11px 14px", background: "#fff1f2", color: "#991b1b", fontWeight: 800 }}>
                  Cerrar sesión
                </button>
              ) : null}
            </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 14 }}>
            {APLOMO_DEMO_ROLE_ACCOUNTS.map((account) => (
              <button
                ...
                style={{
                  textAlign: "left",
                  border: selectedEmail === account.email ? "2px solid #38bdf8" : "1px solid rgba(148,163,184,.35)",
                  borderRadius: 18,
                  padding: 16,
                  background: selectedEmail === account.email ? "rgba(14,165,233,.16)" : "rgba(15,23,42,.78)",
                  color: "#f8fafc",
                  cursor: "pointer"
                }}
              >
                <strong>{account.displayName}</strong>
                <div style={{ color: "#bae6fd", fontSize: 12, fontWeight: 900, marginTop: 4 }}>
                  {account.accountScope === "platform" ? account.platformRole : account.tenantRole}
                </div>
