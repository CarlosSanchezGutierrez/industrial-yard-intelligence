...
export function AplomoDemoAccessLanding() {
  ...
  const [status, setStatus] = useState("Selecciona una cuenta demo e inicia sesion.");
  ...
  async function login() {
    ...
    if (!password.trim()) {
      setStatus("Escribe la contrasena demo.");
      return;
    }

    setIsLoading(true);
    setStatus("Iniciando sesion como " + selectedAccount.displayName + "...");
      ...
      <section style={{ maxWidth: 1180, margin: "0 auto", padding: "56px 20px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1.1fr) minmax(320px, 0.9fr)", gap: 28 }}>
          <div>
            ...
            <p style={{ fontSize: 19, lineHeight: 1.6, color: "#cbd5e1", maxWidth: 720 }}>Plataforma para patios industriales, stockpiles, GPS, dispositivos, datos, gobernanza, analitica e IA futura.</p>
          ...
          <aside style={{ border: "1px solid rgba(148,163,184,.35)", borderRadius: 22, padding: 20, background: "#ffffff", color: "#0f172a" }}>
            ...
            <h2 style={{ margin: "6px 0 12px", fontSize: 25 }}>Iniciar sesion</h2>
            {sessionEmail ? <div style={{ border: "1px solid #bbf7d0", background: "#f0fdf4", color: "#166534", borderRadius: 14, padding: 12, marginBottom: 14 }}>Sesion actual: <strong>{sessionEmail}</strong></div> : null}
            ...
            <label style={{ display: "block", fontSize: 13, fontWeight: 800, marginBottom: 6 }}>Contrasena demo</label>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <button type="button" onClick={() => void login()} disabled={isLoading} style={{ border: 0, borderRadius: 12, padding: "11px 14px", background: "#111827", color: "#ffffff", fontWeight: 900 }}>Iniciar sesion</button>
              {sessionEmail ? <button type="button" onClick={() => void logout()} disabled={isLoading} style={{ border: "1px solid #fecaca", borderRadius: 12, padding: "11px 14px", background: "#fff1f2", color: "#991b1b", fontWeight: 800 }}>Cerrar sesion</button> : null}
            </div>
